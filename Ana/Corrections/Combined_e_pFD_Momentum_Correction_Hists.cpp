...
void getFunctionCD(TGraphErrors * g_mygraph, TF1 * f_myfunc, TFitResultPtr & p_mypoint, double guess[5]){
  ...
  f_myfunc->SetParameter(1,guess[1]);
  f_myfunc->SetParLimits(1,0.0001,0.5);
  f_myfunc->SetParameter(2,guess[2]);
  f_myfunc->SetParLimits(2,-M_PI,M_PI);
  f_myfunc->SetParameter(3,guess[3]);
  f_myfunc->SetParLimits(3,0.0001,0.1);
  f_myfunc->SetParameter(4,guess[4]);
  f_myfunc->SetParLimits(4,-M_PI,M_PI);
...
int main(int argc, char ** argv)
{
  ...
  TF1 * f_phi_corr_binSector_binTheta[6][14];
  TFitResultPtr p_phi_corr_binSector_binTheta[6][14];
  TF1 * f_phi_corr_Combined_binSector_binTheta[6][14];
  TF1 * f_phi_corr_Redo_binSector_binTheta[6][14];
  TFitResultPtr p_phi_corr_Redo_binSector_binTheta[6][14];
  TF1 * f_phi_corr_Redo_Combined_binSector_binTheta[6][14];
  for(int j=1; j<=6; j++){
    for(int i=0; i<14; i++){
      int min = bE_Theta[i];
      ...
      sprintf(temp_name,"f_phi_corr_Combined_sector_%d_theta_%d",j,i);
      ...
      sprintf(temp_name,"f_phi_corr_Redo_sector_%d_theta_%d",j,i);
      f_phi_corr_Redo_binSector_binTheta[j-1][i] = new TF1(temp_name,[&](double *x, double *p){ return FuncPhiDependenceFD(x[0],p[0],p[1],p[2],p[3]); },-40,40,4);
      
      sprintf(temp_name,"f_phi_corr_Redo_Combined_sector_%d_theta_%d",j,i);
      f_phi_corr_Redo_Combined_binSector_binTheta[j-1][i] = new TF1(temp_name,[&](double *x, double *p){ return FuncPhiDependenceFD(x[0],p[0],p[1],p[2],p[3]); },-40,40,4);
  ...
  TF1 * f_phi_corr_binMomCD[12];
  TFitResultPtr p_phi_corr_binMomCD[12];
  TF1 * f_phi_corr_Combined_binMomCD[12];
  for(int i=0; i<12; i++){
    ...
    h_phi_corr_binMomCD[i] = (TH2D*)inFile->Get(temp_name);
    
    g_phi_corr_binMomCD[i] = new TGraphErrors();
    sprintf(temp_name,"g_phi_corr_mom_%d",i);
    g_phi_corr_binMomCD[i]->SetName(temp_name);

    sprintf(temp_name,"f_phi_corr_mom_%d",i);
    f_phi_corr_binMomCD[i] = new TF1(temp_name,[&](double *x, double *p){ return FuncPhiDependenceCD(x[0],p[0],p[1],p[2]); },-180,180,5);    

    sprintf(temp_name,"f_phi_corr_Combined_mom_%d",i);
    f_phi_corr_Combined_binMomCD[i] = new TF1(temp_name,[&](double *x, double *p){ return FuncPhiDependenceCD(x[0],p[0],p[1],p[2]); },-180,180,5);    
  }
  ...
  TFitResultPtr p_params_Redo[6][4];
  for(int j = 0; j < 6; j++){
    TGraph * params[4];
    TF1 * f_params[4];
    double guess[4];
    for(int k = 0; k < 4; k++){
      ...
      f_params[k]->SetParLimits(2,4,100);
      f_params[k]->SetParameter(3,25);
      f_params[k]->SetParLimits(3,10,40);
      if((k==0)||(k==1)){
	f_params[k]->SetParameter(0,0);
	f_params[k]->SetParLimits(0,-10,10);
	...
	f_params[k]->SetParLimits(1,-1000,1000);
      }
      else if(k==3){
	f_params[k]->SetParameter(0,0);
	...
	f_params[k]->SetParLimits(1,-10,10);
      ...
      if(k==0 && j==0){
	f_params[k]->SetParLimits(0,0.0,0.01);
	f_params[k]->SetParLimits(1,-0.1,0.0);
	f_params[k]->SetParLimits(3,16,30);
      }
      if(k==1 && j==0){
	f_params[k]->SetParLimits(2,6,50);
      }

      if(k==2 && j==1){
	f_params[k]->SetParLimits(3,16,50);
      }

      if(k==2 && j==3){
	f_params[k]->SetParLimits(3,15,40);
      }

    }
    
    for(int i = 12; i >= 0; i--){
      getGraph(h_phi_corr_binSector_binTheta[j][i],g_phi_corr_binSector_binTheta[j][i],myCanvas,fileName,ctrX);
    ...
    /////////////
    TGraph * params_Redo[4];
    TGraph * params_mins_Redo[4];
    TGraph * params_maxs_Redo[4];
    TF1 * f_params_Redo[4];
    for(int k = 0; k < 4; k++){
      params_Redo[k] = new TGraph();
      params_mins_Redo[k] = new TGraph();
      params_maxs_Redo[k] = new TGraph();
      sprintf(temp_name,"params_Redo_%d",k);
      f_params_Redo[k] = new TF1(temp_name,[&](double *x, double *p){ return ErfFunc(x[0],p[0],p[1],p[2],p[3]);},5,50,4);
      f_params_Redo[k]->SetParameter(2,20);
      f_params_Redo[k]->SetParLimits(2,4,100);
      f_params_Redo[k]->SetParameter(3,25);
      f_params_Redo[k]->SetParLimits(3,10,40);
      if((k==0)||(k==1)){
	f_params_Redo[k]->SetParameter(0,0);
	//f_params_Redo[k]->SetParLimits(0,-10,10);
	f_params_Redo[k]->SetParameter(1,0);
	//f_params_Redo[k]->SetParLimits(1,-10,10);
      }
      else if(k==2){
	f_params_Redo[k]->SetParameter(0,0);
	//f_params_Redo[k]->SetParLimits(0,-1000,1000);
	f_params_Redo[k]->SetParameter(1,100);
	//f_params_Redo[k]->SetParLimits(1,-1000,1000);
      }
      else if(k==3){
	f_params_Redo[k]->SetParameter(0,0);
	//f_params_Redo[k]->SetParLimits(0,-30,30);
	f_params_Redo[k]->SetParameter(1,0);
	//f_params_Redo[k]->SetParLimits(1,-10,10);
      }
    }
    ...
    for(int i = 12; i >= 0; i--){
      double x = (bE_Theta[i]+bE_Theta[i+1])/2;
      getFunctionRedo(g_phi_corr_binSector_binTheta[j][i],f_phi_corr_Redo_binSector_binTheta[j][i],p_phi_corr_Redo_binSector_binTheta[j][i],f_params,p_params[j],params_mins_Redo,params_maxs_Redo,x,j+1);      
      for(int k = 0; k < 4; k++){
	params_Redo[k]->SetPoint(params_Redo[k]->GetN(),x,p_phi_corr_Redo_binSector_binTheta[j][i]->Parameter(k));
      }      
    }

    for(int k = 0; k < 4; k++){
      p_params_Redo[j][k] = params_Redo[k]->Fit(f_params_Redo[k],"SrBeqn","",5,45);
    }

    //////////////////
    for(int i = 0; i < 13; i++){
      double x = (bE_Theta[i]+bE_Theta[i+1])/2;
      f_phi_corr_Redo_Combined_binSector_binTheta[j][i]->SetLineColor(4);
      for(int k = 0; k < 4; k++){      
	f_phi_corr_Redo_Combined_binSector_binTheta[j][i]->SetParameter(k,f_params_Redo[k]->Eval(x));
      }
    }
    ...
    for(int k = 0; k < 4; k++){
      ...
      f_params[k]->SetLineColor(3);      
      params_Redo[k]->SetLineColor(6);
      f_params_Redo[k]->SetLineColor(6);
      params_mins_Redo[k]->SetLineColor(9);;
      params_maxs_Redo[k]->SetLineColor(9);;

      params_Redo[k]->Draw();
      f_params_Redo[k]->Draw("SAME");      
      params[k]->Draw("SAME");
      f_params[k]->Draw("SAME");      
      params_mins_Redo[k]->Draw("SAME");
      params_maxs_Redo[k]->Draw("SAME");
    }
    ...
    for(int i = 0; i < 13; i++){
      ...
      f_phi_corr_Combined_binSector_binTheta[j][i]->SetLineColor(3);
      f_phi_corr_Combined_binSector_binTheta[j][i]->Draw("SAME");
      //f_phi_corr_Redo_binSector_binTheta[j][i]->Draw("SAME");
      f_phi_corr_Redo_Combined_binSector_binTheta[j][i]->SetLineColor(6);
      f_phi_corr_Redo_Combined_binSector_binTheta[j][i]->Draw("SAME");
    }
  ...
  for(int k = 0; k < 3; k++){
    ...
    f_params_CD[k] = new TF1(temp_name,[&](double *x, double *p){ return FuncMomDependenceCD(x[0],p[0],p[1]);},38,60,2);
    f_params_CD[k]->SetParameter(0,0);
    f_params_CD[k]->SetParLimits(0,-10,10);
    f_params_CD[k]->SetParameter(1,0);
    f_params_CD[k]->SetParLimits(1,-0.1,0.1);
  }
  for(int k = 0; k < 3; k++){
    p_params_CD[k] = params_CD[k]->Fit(f_params_CD[k],"SrBeqn","",38,60);
  }
