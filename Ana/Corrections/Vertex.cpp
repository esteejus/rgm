...
int main(int argc, char ** argv)
{
  ...
  for(int j=1; j<=6; j++){
    for(int i=0; i<14; i++){
      ...
      h_e_vtz_binSector_binTheta[j-1][i] = new TH1D(temp_name,temp_title,50,-5,1);
  ...
  for(int j=1; j<=6; j++){
    for(int i=0; i<14; i++){
      ...
      sprintf(temp_title,"Z Vertex Sector %d (%d< #theta < %d);Z Vertex [cm];Counts",j,min,max);
      h_p_vtz_binSector_binTheta[j-1][i] = new TH1D(temp_name,temp_title,50,-7,1);
  ...
  for(int i=0; i<18; i++){
    ...
    sprintf(temp_title,"Z Vertex (%d< #theta < %d);Z Vertex;Counts",min,max);
    h_vtz_binThetaCD[i] = new TH1D(temp_name,temp_title,50,-4,-1.5);
  ...
  for(int i=0; i<18; i++){
    ...
    h_vtz_binPhiCD[i] = new TH1D(temp_name,temp_title,50,-4,-1.5);
  ...
  while(chain.Next() && (counter<10000000000))
      ...
      if(electrons.size() == 1 && protons.size() >= 0)
	{
	  ...
	  if(binX(bE_Theta,theta_e)!=-1){
	    h_e_vtz_binSector_binTheta[sector_e-1][binX(bE_Theta,theta_e)]->Fill(vtz_e,wep);
	  }
	    
	  if(protons[0]->getRegion()==FD){
	    if(binX(bE_Theta,theta_p)!=-1){
	      h_p_vtz_binSector_binTheta[sector_p-1][binX(bE_Theta,theta_p)]->Fill(vtz_p,wep);
	    }
	  }
	  if(protons[0]->getRegion()==CD){
	    if(binX(bE_ThetaCD,theta_p)!=-1){
	      h_vtz_binThetaCD[binX(bE_ThetaCD,theta_p)]->Fill(vtz_p,wep);
	    }
	    if(binX(bE_PhiCD,phi_p)!=-1){
	      h_vtz_binPhiCD[binX(bE_PhiCD,phi_p)]->Fill(vtz_p,wep);
	    }
	  }
  ...
  for(int j = 0; j < 6 ; j++){
    ...
    for(int i = 0; i < 11; i++){
      ...
      f_thetabin->SetParameter(1,-2);
      f_thetabin->SetParameter(2,1);
      f_thetabin->SetParLimits(2,0.1,2.0);
      TFitResultPtr point = h_e_vtz_binSector_binTheta[j][i]->Fit(f_thetabin,"SrBeqn","",-5,1);
      f_thetabin->Draw("SAME");
      double x = (bE_Theta[i]+bE_Theta[i+1])/2;
      if(point!=-1){
	g_e_sigma[j]->SetPoint(g_e_sigma[j]->GetN(),x,point->Parameter(2));
	//g_ep_sigma[j]->SetPoint(g_ep_sigma[j]->GetN(),x,point->Parameter(2));
      }
    }
    myCanvas->Print(fileName,"pdf");
    myCanvas->Clear();
  ...
  for(int j = 0; j < 6 ; j++){
    ...
    for(int i = 7; i < 14; i++){
      ...
      f_thetabin->SetParameter(2,1);
      f_thetabin->SetParLimits(2,0.1,2.0);
      TFitResultPtr point = h_p_vtz_binSector_binTheta[j][i]->Fit(f_thetabin,"SrBeqn","",-5,1);
      f_thetabin->Draw("SAME");
      double x = (bE_Theta[i]+bE_Theta[i+1])/2;
      if(point!=-1){
  ...
  for(int i = 0; i < 9; i++){
    ...
    f_thetabin->SetParameter(1,-2);
    f_thetabin->SetParameter(2,1);
    f_thetabin->SetParLimits(2,0.01,0.6);
    TFitResultPtr point = h_vtz_binThetaCD[i]->Fit(f_thetabin,"SrBeqn","",-5,1);
    f_thetabin->Draw("SAME");
  ...
  for(int i = 0; i < 9; i++){
    ...
    f_thetabin->SetParameter(2,1);
    f_thetabin->SetParLimits(2,0.01,0.6);
    TFitResultPtr point = h_vtz_binPhiCD[i]->Fit(f_thetabin,"SrBeqn","",-5,1);
    f_thetabin->Draw("SAME");
