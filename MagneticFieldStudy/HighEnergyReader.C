...
void HighEnergyReader(){
  ...
  TFile *tree_file = new TFile("/lustre19/expphy/volatile/clas12/users/esteejus/highenergy/pid/tree_"+outputFile+".root","RECREATE");
  ...
  auto *pid_e_ec = new TH2D("pid_p_ec","EC/p vs e electron",200,0,10,100,0,.6);
  auto *pid_p_fd = new TH2D("pid_p_fd","FD TOF vs p PID proton",500,0,5,500,0,1.3);
  auto *pid_p_cd = new TH2D("pid_p_cd","CD TOF vs p PID proton",200,0,5,200,0,1.3);

  auto *pid_pip_fd = new TH2D("pid_pip_fd","FD TOF vs pip PID", 0,0,5,500,0,1.3);
  auto *pid_pip_cd = new TH2D("pid_pip_cd","CD TOF vs pip PID",200,0,5,200,0,1.3);

  auto *pid_pim_fd = new TH2D("pid_pim_fd","FD TOF vs pim PID", 0,0,5,500,0,1.3);
  auto *pid_pim_cd = new TH2D("pid_pim_cd","CD TOF vs pim PID",200,0,5,200,0,1.3);
  ...
  */
    ...
    TFile *out = new TFile("/lustre19/expphy/volatile/clas12/users/esteejus/highenergy/pid/hist_"+outputFile+".root","RECREATE");
    hmiss->Write();
    pid_e_ec->Write();
    pid_p_fd->Write();
    pid_p_cd->Write();
