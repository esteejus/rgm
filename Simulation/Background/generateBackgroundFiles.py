...
runno_fc_scalars = 15199 # last run with no farady cup scalars

energy_setting = {"6gev":5986, "4gev":4029,"2gev":2070}
#min_currents   = {"6gev":20, "4gev":-1,"2gev":-1} #minium current cutoff
min_currents   = {"6gev":10, "4gev":5,"2gev":0.5}
mag_setting    = {"6gev":"tor-1.00_sol-1.00","4gev":"tor-1.00_sol-1.00","2gev":"tor+0.50_sol-1.00"}
#estimate of number of filtered bkg events in each energy setting
#needed so we dont need to filter all reconstructed events only enough to give us target_events
estimates      = {"6gev":5000, "4gev":2000,"2gev":1000} 


#cooking_paths = glob.glob("/cache/clas12/rg-m/production/pass1/6gev/H/")
...
for cooking_path in cooking_paths:
    ...
    output_dir = "%srgm_fall2021/%s/%s_%sMeV/filtered/" % (output_path,setting,target,str(energy_mev))
...
#    print(cooked_runs)
    paths = [[decoded_path + '0' + str(run),run] for run in cooked_runs]


#    files = [file for file in [glob.glob(path + '/*.hipo') for path,run in paths]]
    files = [file for file in [glob.glob(path + '/*.hipo') for path,runs in paths if runs > runno_fc_scalars]]

#    print(files)

#    files = [[file for file in [glob.glob(path + '/*.hipo')],run] for path,run in paths]
 #   print(files)

#    files = [ x for x in files if (x[1] > runno_fc_scalars)]
