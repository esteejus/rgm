...
int main(int argc, char ** argv)
{
  ...
  TH1D * h_pL = new TH1D("pL","Leading proton momentum",100,0,3);
  hist_list_1.push_back(h_pL);
  TH1D * h_q = new TH1D("q","Magnitude of q",100,0,6);
  hist_list_1.push_back(h_q);
  ...
  TH1D * h_theta_pq = new TH1D("theta_pq","#theta_{p,q}",40,0,40);
  hist_list_1.push_back(h_theta_pq);
...
std::cout << ltheta << endl;
      ...
      if (pL.Theta()*180./M_PI>35) {continue;}
...
//std::cout << mmiss << endl;
    ...
    // implied in lead cuts:
    // if (prot[lead]->getRegion()!=FD) {continue;}
...
//// RECOIL N ////
  ...
  myCanvas->Divide(2,2);
  myCanvas->cd(1);  h_pL->Draw();
  myCanvas->cd(2);  h_q->Draw();
  myCanvas->Print(fileName,"pdf");
  myCanvas->Clear();
  ...
  myCanvas->cd(4);  h_theta_pq->Draw("colz");
  myCanvas->Print(fileName,"pdf");
  myCanvas->Clear();
