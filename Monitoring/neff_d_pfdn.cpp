...
int main(int argc, char ** argv)
{
  ...
  while(chain.Next()==true){
    ...
    if (pmiss.Mag() < 0.243) {continue;} // beta = 0.2, p = 0.1918, beta = 0.25, p = 0.243 // was 0.243
...
*/
  ...
  // print output
  ofstream outp("d_pn/rgm_2gev/p_d_pFDn.txt");
  ...
  // print output
  ofstream outtheta("d_pn/rgm_2gev/theta_d_pFDn.txt");
...
// make histogram projections, fit to 2 Gaussians, subtract background
double * hist_projections_backsub(TCanvas * can, TH2D * hist2d, int num_hist)
{
  double p_start_val[num_hist];
  double x_min = hist2d->GetXaxis()->GetXmin();
  ...
  for (int i=0; i<num_hist; i++)
  {
    ...
    //Ssub[i] = sgfit->Integral(Mlow,Mhigh);
